...
## Import content types / update types

First make sure you have your tokens setup.

```bash
csdx auth:tokens:add -a my-token -k API_KEY> -t <DELIVERY_TOKEN> --delivery
```

Then import the types.

```bash
csdx tsgen -a my-token -o "types/contentstack.ts" --include-editable-tags --include-system-fields
```
