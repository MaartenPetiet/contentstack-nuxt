...
import type { ContentPage } from "../../types/contentstack"; // Import the Page type from the types file

// Define an asynchronous function to fetch page data based on the URL
export const useGetContentPage = async (url: string) => {
  ...
  const { data, status, refresh } = await useAsyncData(`page-${url}`, async () => {
    ...
    const result = await $stack
      .contentType("content_page") // Specify the content type as 'page'
      ...
      .find<ContentPage>(); // Execute the query and cast the result to the Page type
    ...
    if (result.entries) {
      ...
      if ($preview) {
        contentstack.Utils.addEditableTags(entry as ContentPage, 'page', true);
      }
